<template>
    <tr>
        ...
        <th scope="col" class="text-nowrap">
            <a href="#" @click="switchSort('institution')">
                <span>Institution</span>
                <em v-if="sortClmn === 'institution' && sortType === 'desc'" class="bi bi-sort-alpha-up"></em>
                <em v-else class="bi bi-sort-alpha-down"></em>
            </a>
        </th>
        <th scope="col" class="text-nowrap">
            <a href="#" @click="switchSort('program')">
                <span>Program</span>
                <em v-if="sortClmn === 'program' && sortType === 'desc'" class="bi bi-sort-alpha-up"></em>
                <em v-else class="bi bi-sort-alpha-down"></em>
            </a>
        </th>
        <th scope="col" class="text-nowrap">
...
export default {
    ...
    data() {
        return {
            ...
            sortType: 'asc',
            url: '',
            path: 'api/fetch/ministry/claims',
        }
    },
    mounted() {
        ...
        let search = this.url.pathname.split('ministry-search/');
        if (search.length > 1) {
            this.path = search[1];
        }
    },
    methods: {
        switchSort: function (clmn) {
            ...
            let vm = this;
            axios.get('/ministry/api/fetch/claims?page=' + this.page + '&direction=' + this.sortType + '&sort=' + this.sortClmn + '&' + queryString)
